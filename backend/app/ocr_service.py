...
Configuration:
    Set GOOGLE_VISION_API_KEY in your backend/.env file.
"""
...
class GoogleVisionConfig:
    # Read API key exclusively from environment variable — never hardcode secrets.
    API_KEY: str = os.environ.get("GOOGLE_VISION_API_KEY", "")
...
def is_paddleocr_available() -> bool:
    """
    Check if the Google Cloud Vision API key is configured.
...
def extract_text_from_image_bytes(image_buffer) -> str:
    """
    Extract text from an image provided as a BytesIO buffer.
    No data is written to disk — fully memory-only.

    Args:
        image_buffer: io.BytesIO object containing image data.
    ...
    try:
        ...
        raise Exception("Google Cloud Vision request timed out (30s).")
    except requests.exceptions.HTTPError as e:
        status_code = e.response.status_code if e.response else "unknown"
        body = e.response.text if e.response else ""
        raise Exception(f"Google Cloud Vision HTTP {status_code}: {body[:300]}")
    ...
    if "error" in first:
        err = first["error"]
        raise Exception(f"Vision API error {err.get('code', '')}: {err.get('message', 'Unknown error')}")
...
if __name__ == "__main__":
    ...
    if not is_paddleocr_available():
        print("ERROR: GOOGLE_VISION_API_KEY not set in environment.")
        sys.exit(1)
